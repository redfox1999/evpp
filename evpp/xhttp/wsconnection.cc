...
: m_conn(conn) {
    ...
    conn->SetWriteCompleteCallback(std::bind(&WsConnection::onWriteComplete, this, std::placeholders::_1));
...
void WsConnection::onWriteComplete(const TCPConnPtr&) {
    m_sendCallback();
    m_sendCallback = std::bind(&dummyCallback);       
}
...
ssize_t WsConnection::onFramePayloadLen16(const char* data, size_t count)
{
    ...
    uint16_t payloadLen = * (uint16_t*)const_cast<char*>(m_cache.data());
...
ssize_t WsConnection::onFramePayloadLen64(const char* data, size_t count)
{
    ...
    uint64_t payloadLen = *(uint64_t*)const_cast<char*>(m_cache.data());
...
ssize_t WsConnection::handleFrameData(const evpp::TCPConnPtr& conn)
{
...
ssize_t WsConnection::handleMessage(const evpp::TCPConnPtr& conn, uint8_t opcode, const string& data)
{
    switch(opcode)
    {
        case 0x1:
            //utf-8 data
            m_callback(shared_from_this(), Ws_MessageEvent, (const void*)(&data));
            break;    
        case 0x2:
            //binary data
            m_callback(shared_from_this(), Ws_MessageEvent, (const void*)(&data));
        ...
        case 0x8:
            //clsoe
            m_callback(shared_from_this(), Ws_CloseEvent, nullptr);
        ...
        case 0xA:
            //pong
            m_callback(shared_from_this(), Ws_PongEvent, (const void*)(&data));
            break;
        default:
            //error
